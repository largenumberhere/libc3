...
int main() {
  ...
  // check for s1 < s2
  CHECK((libc3Strncmp("a", "c", 2) < 0) && (strncmp("a", "c", 2) < 0));

  // check for s1 == s2
  CHECK(libc3Strncmp("aaaa", "aaaa", 5) == strncmp("aaaa", "aaaa", 5));

  // check for s1 < s2 for length 2
  CHECK((libc3Strncmp("ab", "ca", 3) < 0) && (strncmp("ab", "ca", 3) < 0));

  // check s2 > s2
  CHECK((libc3Strncmp("ba", "ab", 3) > 0) && (strncmp("ba", "ab", 3) > 0));

  // check different lengths
  CHECK((libc3Strncmp("fa", "abcd", 5) > 0) && (strncmp("fa", "abcd", 5) > 0));
